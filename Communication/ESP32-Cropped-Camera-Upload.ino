...
// =============================
// Camera configuration with windowing support
// =============================
void initCamera() {
...
// =============================
// Set camera window (ROI at sensor level)
// =============================
bool setCameraWindow(int roiIndex) {
  sensor_t *s = esp_camera_sensor_get();
  if (!s) return false;
  
  const char* roiNames[] = {"LEFT", "MIDDLE", "RIGHT"};
  
  // For OV2640, we can't do true windowing in JPEG mode
  // Instead we'll use a different approach: capture full image but compress more
  Serial.printf("📐 Preparing %s section\n", roiNames[roiIndex]);
...
// =============================
// Crop JPEG image to ROI
// =============================
bool cropJPEGtoROI(uint8_t* jpegData, size_t jpegLen, int roiIndex, uint8_t** croppedData, size_t* croppedLen) {
  // For JPEG, we need to decode, crop, and re-encode
  // This is complex, so we'll use a simpler approach:
  // Convert JPEG to RGB, crop, convert back to JPEG
  
  // Note: This requires converting JPEG to bitmap, which is memory intensive
  // For production, consider using a proper JPEG cropping library
  
  Serial.println("⚠️ JPEG cropping requires decoding - using alternative approach");
  return false;
}
...
void setup() {
  ...
  Serial.println("\n=== ESP32 Cropped Camera Upload Client ===");
