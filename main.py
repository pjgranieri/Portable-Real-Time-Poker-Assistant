...
Uses enhanced CV models WITHOUT regional cropping (full image analysis)
Monitors server uploads in real-time
Run with: python test_multi_player_cv_integration_no_crop.py
"""
...
class MultiPlayerCVInputInterface:
    """Computer Vision interface with enhanced models - NO CROPPING (full image analysis)"""
    ...
    def detect_cards(self, image_path):
        """
        Detect cards in FULL image (NO CROPPING)
        YOLO returns: "AC", "KD", "10S" (VALUE+SUIT)
        """
        print(f"  [CV] Analyzing FULL image for cards")
    ...
    def wait_for_action(self, player_enum, call_value, bankroll, max_attempts=5):
        """
        Wait for player action - NO CROPPING (analyzes FULL image)
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                ...
                # STEP 1: Analyze action on FULL IMAGE with ENHANCED analyzer
                print(f"  [CV] Analyzing FULL IMAGE with EnhancedActionAnalyzer...")
...
class TestMultiPlayerCVIntegration:
    ...
    def run(self):
        """Main game loop"""
        print("\n" + "="*60)
        print(" MULTI-PLAYER POKER GAME - CV INTEGRATION (NO CROP)")
        print("="*60)
        ...
        print(" Cropping: NONE - All analysis on FULL images")
        print("="*60)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Multi-player CV Poker Game Integration Test (NO CROPPING)')
